    ...
    <main class="mx-auto flex w-full max-w-7xl flex-1 flex-col gap-8 px-6 py-8 md:px-10 md:py-10">
      <header class="page-banner flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between" :style="{ '--works-banner': `url(${communityVisuals.plazaImage})` }">
        ...
        <div class="flex flex-wrap items-center gap-3">
          <button
            @click="goLeaderboard"
            class="flex h-11 items-center gap-2 rounded-full border border-[rgba(180,140,94,0.24)] bg-[linear-gradient(180deg,rgba(255,250,243,0.92),rgba(246,233,214,0.82))] px-5 text-sm font-medium text-slate-600 shadow-[0_10px_24px_rgba(92,59,35,0.05)] transition-all hover:border-primary/30 hover:text-primary"
          >
      ...
      <div class="flex flex-col justify-between gap-6 border-b border-slate-200/80 pb-px md:flex-row md:items-end">
        <div class="flex gap-8 md:gap-10">
          <button
            v-for="filter in filterOptions"
            :key="filter.value"
            class="relative pb-4 font-serif text-lg transition-colors"
            :class="currentFilter === filter.value ? 'font-bold text-primary' : 'text-slate-500 hover:text-primary'"
            @click="currentFilter = filter.value"
          >
            {{ filter.label }}
            <div v-if="currentFilter === filter.value" class="absolute bottom-0 left-1/2 h-1 w-10 -translate-x-1/2 rounded-full bg-primary"></div>
          </button>
        </div>
        <div class="pb-4">
          <label class="group relative flex items-center">
            <div class="absolute left-4 text-slate-400 transition-colors group-focus-within:text-primary">
              <span class="material-symbols-outlined text-xl">search</span>
            </div>
            <input
              v-model.trim="searchQuery"
              class="w-full rounded-full border border-transparent bg-slate-200/40 pl-12 pr-4 text-sm outline-none transition-all placeholder:text-slate-400 focus:border-primary/30 focus:ring-0 md:w-72"
              placeholder="搜索曲目或唱者"
            />
          </label>
        </div>
